...
| `parallelRouteTransform`         | `undefined` | Controls worker-thread route transforms. `undefined` auto-enables workers for 256+ routes, `true` uses Rspack's default worker count, a positive integer sets the maximum worker count, and `false` keeps transforms inline.     |

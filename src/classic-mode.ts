...
export const createClassicWebRouteEntries = ({
  ...
  // Route files and generated chunks share the same entry namespace. Routes
  // sharing a file may reuse an entry, but distinct files must never overwrite it.
  const routeFileByEntryName = new Map<string, string>();
  const reserveEntryName = (entryName: string, routeFilePath: string) => {
    ...
    if (
      ...
      throw new Error(
        `[rsbuild-plugin-react-router] Route files ${JSON.stringify(existingRouteFile)} and ${JSON.stringify(routeFilePath)} both resolve to the entry name ${JSON.stringify(entryName)}. Rename one of them so its route entry and generated chunk names are unique.`
      );
    }
  ...
  const webRouteEntries = Object.values(routes).reduce(
    (acc, route) => {
      ...
      reserveEntryName(entryName, routeFilePath);
      ...
      if (isBuild && splitRouteModules && route.id !== 'root') {
        ...
        for (const exportName of routeChunkExportNames) {
          ...
          reserveEntryName(chunkEntryName, routeFilePath);

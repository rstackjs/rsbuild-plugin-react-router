...
describe('parallel route transforms', () => {
  it.each([
    ...
    [24, { routeCount: 1024, splitRouteModules: true }, 16],
  ...
  it.each([
    ...
    [10, 8],
    [24, 8],
  ])('caps medium split route module builds at eight workers', (cpus, workers) => {
    expect(
  ...
  it.each([
    ...
    [18, 16],
    [24, 16],
  ])('caps large split route module builds at sixteen workers', (cpus, workers) => {
    expect(
      getDefaultWorkerCount(cpus, {
        routeCount: 1024,
        splitRouteModules: true,

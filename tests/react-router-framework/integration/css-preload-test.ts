...
for (const vanillaExtract of [false, true]) {
  test(`CSS preloads cannot revive edited bytes after exact restoration (vanillaExtract=${vanillaExtract})`, async ({
    page,
  }) => {
    const port = await getPort();
    const original = vanillaExtract
      ? `import { style } from '@vanilla-extract/css'; export const sentinel = style({ color: 'rgb(255, 0, 0)', outline: 'none' });`
      : '.sentinel { color: rgb(255, 0, 0); outline: none; }';
    const edited = original
      .replace('rgb(255, 0, 0)', 'rgb(0, 0, 255)')
      .replace('none', '2px dashed');
    const styleFile = vanillaExtract ? 'app/style.css.ts' : 'app/style.css';
    const cwd = await createProject({
      'rsbuild.config.ts': await rsbuildConfig.basic({ port, vanillaExtract }),
      [styleFile]: original,
      'app/routes/_index.tsx': `
      ${vanillaExtract ? "import { sentinel } from '../style.css';" : "import '../style.css';"}
      import { useEffect, useState } from 'react';
      export default function Index() {
        const [ready, setReady] = useState(false);
        useEffect(() => setReady(true), []);
        return <><input aria-label="Draft" /><h1 className={${vanillaExtract ? 'sentinel' : '"sentinel"'}} data-ready={ready}>CSS</h1></>;
      }
    `,
    });
